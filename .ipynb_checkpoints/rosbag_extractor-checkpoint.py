...
topics = [
    ...
    "/turbot/imagenex_echosounder_down/profile_range_filtered"
    "/turbot/modem_raw",
    ...
    "/turbot/navigator/altitude/echo",
    "/turbot/navigator/altitude/dvl",
    "/turbot/navigator/altitude/ekf_alt/odometry"
]
...
def altitude_z(msg):
    x = Altitude()
    x.epoch_timestamp = stamp_to_epoch(msg.header.stamp)
    x.altitude = msg.pose.pose.position.z
    return x
...
def parse_bagfile(bagfile, force_overwrite):
    ...
    altitude_dvl=[]
    ekf_alt=[]
    echo_range_raw,echo_range_filtered=[],[]
    dvl=[]
    navigation_data=[]
    ...
    for topic, msg, t in bag.read_messages(topics=topics):
        ...
        if "/turbot/navigator/imu" in topic:
            ...
            echo_range_raw.append(x)

        elif "/turbot/imagenex_echosounder_down/profile_range_filtered" in topic:
            print("HEEEEEEEEEEEEEEEEEEEEEEEY")
            json_data, x = altitude_to_json(msg)
            echo_range_filtered.append(x)
            ...
            x=navigation_from_ros(msg)
            navigation_data.append(x)

        elif "/turbot/navigator/altitude/echo" in topic:
            x=altitude_z(msg)
            # print(x,"altitude_echo")
            altitude_echo.append(x)
        elif "/turbot/navigator/altitude/dvl" in topic:
            x=altitude_z(msg)
            altitude_dvl.append(x)
        elif "/turbot/navigator/altitude/ekf_alt/odometry" in topic:
            x=altitude_z(msg)
            ekf_alt.append(x)
    ...
    echo_range_raw_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in echo_range_raw
    ]
    echo_range_raw_z= [x.altitude for x in echo_range_raw]

    echo_range_filtered_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in echo_range_filtered
    ]
    echo_range_filtered_z= [x.altitude for x in echo_range_filtered]

    nav_data_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in navigation_data
    ]

    nav_north = [x.position_north for x in navigation_data]
    nav_east = [x.position_east for x in navigation_data]
    nav_down = [x.position_down for x in navigation_data]
    
    nav_altitude = [x.altitude for x in navigation_data]

    nav_roll = [x.orientation_roll for x in navigation_data]
    nav_pitch = [x.orientation_pitch for x in navigation_data]
    nav_yaw = [x.orientation_yaw for x in navigation_data]

    #----------------------------------------------------
    # print("altitude_echo_filt",echo_range_filtered_z)

    altitude_echo_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in altitude_echo
    ]
    altitude_echo_z= [x.altitude for x in altitude_echo]

    altitude_dvl_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in altitude_dvl
    ]
    altitude_dvl_z= [-x.altitude for x in altitude_dvl]

    ekf_alt_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in ekf_alt
    ]
    ekf_alt_z= [x.altitude for x in ekf_alt]

    dvl_dvl_t = [
        datetime.datetime.utcfromtimestamp(x.epoch_timestamp)
        for x in dvl
    ]
    dvl_dvl_z= [x.altitude for x in dvl]
    ...
    fig.add_trace(
        go.Scatter(x=altitude_echo_t, y=altitude_echo_z, name="altitude_echo"),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(x=altitude_dvl_t, y=altitude_dvl_z, name="altitude_dvl"),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(x=ekf_alt_t, y=ekf_alt_z, name="ekf_alt"),
        row=1,
        col=1,
    )

    fig.add_trace(
        go.Scatter(x=echo_range_raw_t, y=echo_range_raw_z, name="echo_range_raw"),
        row=1,
        col=2,
    )
    fig.add_trace(
        go.Scatter(x=echo_range_filtered_t, y=echo_range_filtered_z, name="echo_range_filtered"),
        row=1,
        col=2,
    )

    fig.add_trace(
        go.Scatter(x=dvl_dvl_t, y=dvl_dvl_z, name="dvl_dvl"),
        row=1,
        col=2,
    )
    ...
    html_fn = bagfile_fn.parent / (bagfile_fn.stem + "altitude_depth.html")
